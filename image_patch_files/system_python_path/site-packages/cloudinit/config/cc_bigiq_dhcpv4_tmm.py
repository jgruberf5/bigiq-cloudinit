...
def get_linux_interfaces():
    ...
    interfaces = subprocess.Popen(
        "%s link | %s '%s[1-9]' | %s -d':' -f2 | %s -d ' '" % (
            SYSCMDS['ip'], SYSCMDS['egrep'], SYSDEVPREFIX, SYSCMDS['cut'], SYSCMDS['tr']),
...
def resolve_resources(rd_enabled=True, default_route_interface=None,
    ...
    try:
        ...
        if 'interface-mtu' in mgmt_data:
            ...
            bigiq_onboard_utils.force_mgmt_mtu(m_mtu)
...
def create_mgmt_init_script(resources):
    ...
    with open(mgmt_onboard_script, 'w') as mgmt_script:
        ...
        mgmt_script.write("%s modify sys global-settings mgmt-dhcp disabled\n" % SYSCMDS['tmsh'])
        mgmt_script.write("%s 5\n" % SYSCMDS['sleep'])
        mgmt_script.write(
            "%s create sys management-ip %s/%s\n" % (
                SYSCMDS['tmsh'], resources['mgmt_address'], resources['mgmt_netmask']))
        if resources['mgmt_gateway']:
            mgmt_script.write("%s create sys management-route default gateway %s\n" % (
        ...
        mgmt_script.write(
            "%s 'mgmt interface configured: %s/%s'\n" % (
                SYSCMDS['echo'], resources['mgmt_address'], resources['mgmt_netmask']))
