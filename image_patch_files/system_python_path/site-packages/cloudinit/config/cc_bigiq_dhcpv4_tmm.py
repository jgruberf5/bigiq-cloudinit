...
def get_linux_interfaces():
    ...
    interfaces = subprocess.Popen(
        ...
        (SYSCMDS['ip'], SYSCMDS['egrep'], SYSDEVPREFIX, SYSCMDS['cut'],
         SYSCMDS['tr']),
...
def create_bigiq_init_script(resources, license_key, node_type):
    ...
    with open(ANSIBLE_VAR_FILE, 'w') as ansible_vars:
        ...
        ansible_vars.write('admin_password: %s\n' % resources['admin_password'])
        ansible_vars.write('root_password: %s\n' % resources['root_password'])
...
def create_post_onboard():
    ...
    with open(onboard_script, 'w') as obs:
        ...
        obs.write("    while [ $checks -lt 120 ]; do\n")
        obs.write(
            "        started=$(%s -su 'admin:' http://localhost:8100/shared/echo | %s '\"stage\":\"STARTED\"' | %s -l 2>/dev/null)\n"
...
def handle(name, userdata, cloud, log, args):
    ...
    if enabled:
        ...
        resources['admin_password'] = admin_password
        resources['root_password'] = root_password
        ...
        if admin_password and root_password:
            try:
            ...
            onboard_status = ERROR
            LOG.error(
                'onboarding BIG-IQ requires both admin and root passwords to be set.'
            )
            if phone_home_url:
                bigiq_onboard_utils.phone_home(phone_home_url, onboard_status,
                                               phone_home_url_verify_tls)
