        ...
        for link in resources['links']:
            if resources['links'][link]['interface'] == resources['device_discovery_interface']:
                if link in resources['selfips']:
                    ...
                    cmi_script.write("%s show net self %s > /dev/null 2>&1; while [ $? -ne 0 ]; do %s 2; %s show net self %s > /dev/null 2>&1; done\n" % (
                        SYSCMDS['tmsh'], selfip['selfip_name'], SYSCMDS['sleep'], SYSCMDS['tmsh'], selfip['selfip_name']))
                    ...
                    LOG.warn('device discovery configuration incomplete')
        cmi_script.write(
