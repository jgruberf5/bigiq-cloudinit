...
def create_bigiq_init_script(resources, license_key, node_type):
    ...
    with(open(ANSIBLE_VAR_FILE), 'w') as ansible_vars:
        ...
        ansible_vars.write('device_discvoery_address: %s\n' % resources['device_discovery_address'])
    return resources
...
def create_onboard_artifacts(resources, license_key, node_type, post_onboard_commands=None):
...
def handle(name, userdata, cloud, log, args):
    ...
    if enabled:
        ...
        create_onboard_artifacts(resources, license_key, node_type, post_onboard_commands)
        create_onboard()
        create_post_onboard()
