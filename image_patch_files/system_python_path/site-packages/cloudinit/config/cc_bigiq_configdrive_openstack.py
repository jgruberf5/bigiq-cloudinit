...
def resolve_resources(rd_enabled=True, default_route_interface=None,
    ...
    if not device_discovery_interface:
        device_discovery_interface = DEFAULT_CONFIGSYNC_INTERFACE
...
def create_cmi_init_script(resources):
    ...
    with open(sync_onboard_script, 'w') as cmi_script:
        services = resources['services']
        cmi_script.write("%s mv cm device $(%s list cm device | %s 'cm device' | %s -d ' ' -f3) %s\n" %
                         (SYSCMDS['tmsh'], SYSCMDS['tmsh'], SYSCMDS['grep'], SYSCMDS['cut'], services['hostname']))
        for link in resources['links']:
            if resources['links'][link]['interface'] == resources['device_discovery_interface']:
                if link in resources['selfips']:
                    ...
                    cmi_script.write("%s show net self %s > /dev/null 2>&1; while [ $? -ne 0 ]; do sleep 2; %s show net self %s > /dev/null 2>&1; done\n" % (
                        SYSCMDS['tmsh'], selfip['selfip_name'], SYSCMDS['tmsh'], selfip['selfip_name']))
                    cmi_script.write("%s modify cm device %s configsync-ip %s unicast-address { { effective-ip %s effective-port 1026 ip %s } { effective-ip management-ip effective-port 1026 ip management-ip } } mirror-ip %s\n" % (
                        SYSCMDS['tmsh'], services['hostname'], sip, sip, sip, sip))
                else:
                    LOG.warn('configsync configuration incomplete')
        cmi_script.write(
            "%s 'CMI device discovery interface configured'\n" % SYSCMDS['echo'])
        return resources
...
def onboard(post_onboard_enabled,
    ...
    if onboard_status == SUCCESS:
        if post_onboard_enabled:
            post_onboard_script = POST_ONBOARD_CMD_FILE_DIR + '/onboard.sh'
            with open(LOG_FILE, 'a+') as onboardlog:
                LOG.info('running post onboarding scripts')
                subprocess.call([SYSCMDS['nohup'], 'sh', '-c', onboard_script,
                                 '&'], stdout=onboardlog, stderr=onboardlog)
            end_time = time.time() + ONBOARD_TIMEOUT
            while (end_time - time.time()) > 0:
