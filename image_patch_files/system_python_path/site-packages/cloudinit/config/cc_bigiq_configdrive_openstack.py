...
ANSIBLE_VAR_FILE = '/var/lib/cloud/bigiq_ansible_onboard.yml'
ANSIBLE_PLAYBOOK_DIR = '/var/lib/cloud/ansible'
ANSINLE_PLAYBOOK = 'onboard.yml'
...
def resolve_resources(rd_enabled=True, default_route_interface=None,
    ...
    for net in os_nmd['networks']:
        ...
        if net['type'] == 'ipv4_dhcp':
            if net['link'] == m_l_id:
                ...
                interface = "%s%d" % (SYSDEVPREFIX, links[net['link']]['interface_index'])
                ...
                try:
                    if bigiq_onboard_utils.make_dhcp4_request(interface, dhcp_timeout):
                        ...
                        if s_gw and s_gw != m_gw:
                            ...
                            if device_discovery_interface == links[net['link']]['interface']:
                                LOG.debug(
                                    'device_discovery_interface discovered with DHCPv4 as link: %s', net['link'])
                            ...
                            if route['network'] == '0.0.0.0' or route['network'] == '::':
                                if route['gateway'] != m_gw:
                                    ...
                                    if default_route_interface == links[net['link']]['interface']:
                                        ...
                                        number_of_default_routes += 1
                                    if device_discovery_interface == links[net['link']]['interface']:
                                        LOG.debug('device_discovery_interface discovered as interface: %s',
    ...
    for service in os_nmd['services']:
        if service['type'] == 'dns' and (bigiq_onboard_utils.is_v6(service['address']) or bigiq_onboard_utils.is_v4(service['address'])):
            LOG.debug('discovered DNS service configuration')
            dns_services.append(service['address'])
        if service['type'] == 'ntp' and (bigiq_onboard_utils.is_v6(service['address']) or bigiq_onboard_utils.is_v4(service['address'])):
...
def create_mgmt_init_script(resources):
    ...
    with open(mgmt_onboard_script, 'w') as mgmt_script:
        ...
        mgmt_script.write(
            "%s create sys management-ip %s/%s\n" % (SYSCMDS['tmsh'],
        ...
        mgmt_script.write("%s link set %s mtu %s\n" %
                          (SYSCMDS['ip'], bigiq_onboard_utils.MGMT_DEV_NAME, resources['mgmt_mtu']))
        mgmt_script.write(
            "%s '%s link set %s %s' >> /config/startup\n" % (
                SYSCMDS['echo'], SYSCMDS['ip'], bigiq_onboard_utils.MGMT_DEV_NAME, resources['mgmt_mtu']))
        mgmt_script.write(
            "%s 'mgmt interface configured: %s/%s'\n" % (SYSCMDS['echo'],
                                                         resources['mgmt_address'],
                                                         resources['mgmt_netmask']))
    ...
    with open(network_onboard_script, 'w') as net_script:
        ...
        for l_id in resources['links']:
            ...
            if l_id == resources['mgmt_link_id']:
                continue
            if not link['tagged']:
                net_script.write("%s create net vlan %s mtu %s interfaces replace-all-with { %s } tag %s\n" % (
                    SYSCMDS['tmsh'], link['net_name'], link['mtu'], link['interface'], link['segmentation_id']))
            else:
                net_script.write("%s create net vlan %s mtu %s interfaces replace-all-with { %s { tagged } } tag %s\n" % (
                    SYSCMDS['tmsh'], link['net_name'], link['mtu'], link['interface'], link['segmentation_id']))
            if link['route_domain'] > 0:
                net_script.write("%s create net route-domain %s { id %s vlans add { %s } }\n" % (
                    SYSCMDS['tmsh'], link['route_domain'], link['route_domain'], link['net_name']))
        # write out L3 network configuration
        for n_link in resources['selfips']:
            selfip = resources['selfips'][n_link]
            net_script.write("%s create net self %s address %s/%s vlan %s allow-service all\n" %
                             (SYSCMDS['tmsh'], selfip['selfip_name'], selfip['ip_address'], selfip['netmask'], selfip['net_name']))

        for n_link in resources['routes']:
            for route in resources['routes'][n_link]:
                if not route['exclude']:
                    net_script.write("%s create net route %s network %s/%s gw %s\n" % (
                        SYSCMDS['tmsh'],
                        route['route_name'],
                        route['network'],
                        route['netmask'],
...
def creat_services_init_script(resources):
    ...
    with open(services_onboard_script, 'w') as services_script:
        ...
        services_script.write("%s modify sys global-settings hostname %s.%s\n" %
...
def create_bigiq_init_script(resources, license_key, node_type, license_pool_name, license_offerings):
    ...
    with open(sync_onboard_script, 'w') as cmi_script:
        ...
        cmi_script.write("%s %s\n" % (SYSCMDS['ansible-playbook'], ANSINLE_PLAYBOOK))
    with open(ANSIBLE_VAR_FILE, 'w') as ansible_vars:
        ...
        if license_offerings:
            if not license_pool_name:
                license_pool_name = 'REGKEYPOOL'
                ansible_vars.write('license_offerings:\n')
                for offering in license_offerings:
                    ansible_vars.write('  - %s\n' % offering)
        if license_pool_name:
            ansible_vars.write('license_pool_name: %s\n' % license_pool_name)
    return resources
...
def create_onboard_artifacts(resources, license_key, node_type, license_pool_name, license_offerings, post_onboard_commands=None):
    ...
    create_bigiq_init_script(resources, license_key, node_type, license_pool_name, license_offerings)
...
def create_onboard():
    ...
    with open(onboard_script, 'w') as obs:
        ...
        obs.write("    checks=0\n")
        obs.write("    while [ $checks -lt 120 ]; do\n")
        obs.write("        started=$(%s -su 'admin:' http://localhost:8100/shared/echo | %s '\"stage\":\"STARTED\"' | %s -l 2>/dev/null)\n" % (SYSCMDS['curl'], SYSCMDS['grep'], SYSCMDS['wc']))
        obs.write("        if [ $started == 1 ]; then\n")
        obs.write("            break\n")
        obs.write("        fi\n")
        obs.write("        %s 'waiting for iControl to start'\n" % SYSCMDS['echo'])
        obs.write("        let checks=checks+1\n")
        obs.write("        sleep 10\n")
        obs.write("    done\n")
        obs.write("}\n\n")
...
def is_startup_injected(script_name):
    ...
    injected_already = subprocess.Popen(
        "%s /config/startup | %s " + script_name + " | %s -l" % (SYSCMDS['cat'], SYSCMDS['grep'], SYSCMDS['wc']),
...
def run_post_onboard_commands():
    ...
    with open(LOG_FILE, 'a+') as onboardlog:
        ...
        subprocess.call([SYSCMDS['nohup'], 'sh', '-c', post_onboard_script, '&'],
...
def onboard(post_onboard_enabled,
...
def handle(name, userdata, cloud, log, args):
    ...
    if enabled:
        ...
        license_pool_name = None
        if 'license_pool_name' in userdata[tag]:
            license_pool_name = userdata[tag]['license_pool_name']
        license_offerings = []
        if 'license_offerings' in userdata[tag]:
            license_offerings = userdata[tag]['license_offerings']
        ...
        create_onboard_artifacts(resources, license_key, node_type, license_pool_name, license_offerings, post_onboard_commands)
        create_onboard()
        create_post_onboard()
