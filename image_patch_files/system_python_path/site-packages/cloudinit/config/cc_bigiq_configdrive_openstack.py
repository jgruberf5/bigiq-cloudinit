...
def create_mgmt_init_script(resources):
    ...
    with open(mgmt_onboard_script, 'w') as mgmt_script:
        ...
        mgmt_script.write("ip link set eth0 mtu %s\n" %
                          resources['mgmt_mtu'])
        mgmt_script.write(
            "echo 'ip link set mgmt eth0 %s' >> /config/startup\n" % resources['mgmt_mtu'])
        mgmt_script.write(
...
def create_tmm_net_init_script(resources):
    ...
    network_onboard_script = TMSH_CMD_FILE_DIR + '/001_network_setup.sh'
...
def creat_services_init_script(resources):
    ...
    services_onboard_script = TMSH_CMD_FILE_DIR + '/002_services_setup.sh'
...
def create_cmi_init_script(resources):
    ...
    sync_onboard_script = TMSH_CMD_FILE_DIR + '/003_device_discovery_setup.sh'
    ...
    with open(sync_onboard_script, 'w') as cmi_script:
        ...
        cmi_script.write(
            "echo 'CMI device discovery interface configured'\n")
        return resources
...
def onboard(post_onboard_enabled,
    ...
    if onboard_status == SUCCESS and post_onboard_enabled:
        post_onboard_script = POST_ONBOARD_CMD_FILE_DIR + '/onboard.sh'
        with open(LOG_FILE, 'a+') as onboardlog:
