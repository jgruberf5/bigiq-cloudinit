...
ip - the CIDR representation of the mgmt address you want assigned
gw - the default gateway for the mgmt network
mtu - the mgmt interface MTU to set
...
def create_onboard_artifacts(mgmt_cidr, mgmt_gw, mgmt_mtu, hostname,
                             ...
                             license_key, node_type, post_onboard_commands):
    ...
    with open(network_onboard_script, 'w') as mgmt_script:
        ...
        if mgmt_mtu:
            ...
            mgmt_script.write(
                "%s '%s link set %s mtu %s' >> /config/startup\n" %
                (SYSCMDS['echo'], SYSCMDS['ip'],
                 bigiq_onboard_utils.MGMT_DEV_NAME, mgmt_mtu))
        if hostname:
            mgmt_script.write("%s modify sys global-settings hostname %s\n" %
        ...
        if ntpservers:
            ...
            mgmt_script.write("%s modify sys ntp timezone %s\n" %
    ...
    if license_key:
        ...
        sync_onboard_script = TMSH_CMD_FILE_DIR + '/002_device_discovery_setup.sh'
        ...
        with open(ANSIBLE_VAR_FILE, 'w') as ansible_vars:
            ...
            ansible_vars.write('device_discovery_interface: mgmt\n')
            ansible_vars.write('device_discovery_address: %s\n' %
                               mgmt_cidr.split('/')[0])

    if post_onboard_commands:
        post_onboard_commands_script = TMSH_CMD_FILE_DIR + '/003_post_onboard_commands.sh'
...
def onboard(phone_home_url, phone_home_url_verify_tls, phone_home_url_metadata,
            phone_home_cli):
    ...
    if phone_home_url:
        bigiq_onboard_utils.phone_home(phone_home_url, False, False,
                                       onboard_status,
...
def handle(name, userdata, cloud, log, args):
    ...
    if enabled:
        ...
        try:
            onboard(phone_home_url, phone_home_url_verify_tls,
