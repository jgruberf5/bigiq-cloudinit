...
Any user_data vars supplied will be written to the file:
    ...
    /var/lib/cloud/ansible/[name]/[name]_vars.yml
...
def create_playbook_artifact(index, name, vars):
    ...
    tmsh_cmd_dir_exists()
    playbook_script = TMSH_CMD_FILE_DIR + '/00' + str(index) + '_playbook.sh'
    playbook_yaml_file = ANSIBLE_PLAYBOOK_DIR + '/' + name + '/' + name + '.yml'
    playbook_var_file = ANSIBLE_PLAYBOOK_DIR + '/' + name + '/' + name + '_vars.yml'
    if os.path.isfile(playbook_script):
...
def handle(name, userdata, cloud, log, args):
    ...
    if enabled:
        ...
        for indx, pb in enumerate(playbooks, start=1):
            LOG.info('create playbook runner for playbook: %s' % (pb))
            if 'name' in pb:    
                vars = {}
                if 'vars' in pb:
                    vars = pb['vars']
