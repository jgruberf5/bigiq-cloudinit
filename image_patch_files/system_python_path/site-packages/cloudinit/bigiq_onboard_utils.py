...
def force_tmm_down():
    ...
    fnull = open(os.devnull, 'w')
    subprocess.call(['/usr/bin/pkill', 'tmm'], stdout=fnull, stderr=fnull)
    subprocess.call(['/usr/bin/bigstart', 'shutdown', 'tmm'],
...
def start_tmm():
    ...
    fnull = open(os.devnull, 'w')
    subprocess.call(['/usr/bin/bigstart', 'startup', 'tmm'],
                    stdout=fnull, stderr=fnull)
    time.sleep(BIGSTART_DELAY)
...
def bigstart_restart(service_name=None):
    ...
    if not service_name:
        service_name = ''
    subprocess.call(['/usr/bin/bigstart', 'resstart', service_name],
                    stdout=fnull, stderr=fnull)
    time.sleep(BIGSTART_DELAY)
...
def make_dhcp4_request(interface, timeout=120):
    ...
    if os.path.isfile(lease_file):
        del_file(lease_file)
    subprocess.call(['/usr/bin/pkill', 'dhclient'], stdout=fnull)
...
def onboard():
    if not os.path.exists(FIRST_BOOT_FILE):
        if onboard_configdrive():
            ...
            LOG.error('BIG-IQ cloud-init only accepts ConfigDrive as a data source and none was found')
            touch_file(FIRST_BOOT_FILE)
    else:
