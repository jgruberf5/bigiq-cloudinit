...
def onboard_configdrive():
    ...
    if dev:
        ...
        if os.path.exists(networkdatapath):
            ...
            networkdata = f.read()
            f.close()
        create_cloudinit_resources(dev, systemid, userdata, metadata,
                                   networkdata, None)
...
def onboard_nocloud():
    ...
    if dev:
        ...
        if os.path.exists(vendordatapath):
            ...
            vendordata = f.read()
            f.close()
        create_cloudinit_resources(dev, systemid, userdata, metadata, None,
                                   vendordata)
        ummount_configdrive(mountpath)
        os.rmdir(mountpath)
        return True
...
def create_cloudinit_resources(dev, systemid, userdata, metadata, networkdata,
    ...
    with open('/opt/cloud/data/previous-datasource', 'w') as ds:
        ds.write("DataSourceNoCloud: DataSourceNoCloud [seed=%s][dsmode=net]" %
                 dev)
    ...
    if vendordata:
        with open("/opt/cloud/instances/%s/vendor-data.txt" % systemid,
                  'w') as ds:
            LOG.inf("writing vendor-data.txt: %d bytes" % len(vendordata))
...
def process_text_metadata():
    metadatapath = '/opt/cloud/instance/meta_data'
    if os.path.extsep(metadatapath):
        with open(metadatapath, 'r') as md:
            lines = md.read()
            for line in lines.splitlines():
                if line.startswith('local-hostname'):
                    set_hostname(line.split(': ')[1])
