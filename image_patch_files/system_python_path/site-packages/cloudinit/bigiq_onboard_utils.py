...
def onboard_configdrive():
    ...
    LOG.info('attempting to mount %s as a ConfigDrive data source')
    if dev:
...
def create_cloudinit_resources(dev, systemid, userdata, metadata, networkdata):
    ...
    with open("/opt/cloud/instances/%s/user-data.txt" % systemid, 'w') as ds:
        LOG.info('writing user-data: %d bytes' % len(metadata))
        ds.write(userdata)
    with open("/opt/cloud/instances/%s/metadata.json" % systemid, 'w') as ds:
        LOG.info('writing metadata.json: %d bytes' % len(metadata))
        ds.write(metadata)
    with open("/opt/cloud/instances/%s/network_data.json" % systemid, 'w') as ds:
        LOG.info('writing network_data.json: %d bytes' % len(metadata))
