...
def is_mgmt_ip():
    ...
    mgmt_ip = subprocess.Popen(
        "%s addr show %s | %s '^\\s*inet '| %s -v 169.254 | %s -l" % (
            SYSCMDS['ip'], MGMT_DEV_NAME, SYSCMDS['grep'], SYSCMDS['grep'], SYSCMDS['wc']),
...
def is_mgmt_default_gateway():
    ...
    mgmt_gw = subprocess.Popen(
        "%s -n | %s '^0.0.0.0'| %s %s | %s -l" % (
            SYSCMDS['route'], SYSCMDS['grep'], SYSCMDS['grep'], MGMT_DEV_NAME, SYSCMDS['wc']),
        stdout=subprocess.PIPE, stderr=fnull, shell=True
    ).communicate()[0].replace('\n', '')
    if int(mgmt_gw) == 1:
        return True
...
def get_tmos_version():
    ...
    version = subprocess.Popen(
        "%s /VERSION | %s -i sequence | %s -d':' -f2 | %s '[A-Z]' '[a-z]' | %s -d '[:space:]'" % (
            SYSCMDS['cat'], SYSCMDS['grep'], SYSCMDS['cut'], SYSCMDS['tr'], SYSCMDS['tr']),
        stdout=subprocess.PIPE, stderr=fnull, shell=True
    ).communicate()[0].replace('\n', '')
    return version
...
def get_dmi_uuid():
    ...
    uuid = subprocess.Popen(
        "%s | %s -i UUID | %s -d':' -f2 | %s '[A-Z]' '[a-z]' | %s -d '[:space:]'" % (
            SYSCMDS['dmidecode'], SYSCMDS['grep'], SYSCMDS['cut'], SYSCMDS['tr'], SYSCMDS['tr']),
        stdout=subprocess.PIPE, stderr=fnull, shell=True
    ).communicate()[0].replace('\n', '')
    return uuid
...
def set_password(username=None, password=None):
    ...
    if username:
        if not password:
            ...
            cmd = subprocess.Popen(SYSCMDS['chpasswd'], stdin=stdin, stdout=stdout, stderr=stderr)
...
def wait_for_mgmt_dhcp(timeout=None):
    ...
    while (end_time - time.time()) > 0:
        if os.path.isfile(MGMT_DHCP_LEASE_FILE) and os.path.getsize(MGMT_DHCP_LEASE_FILE) > 0:
            return True
...
def is_tmm():
    ...
    tmm_running = int(subprocess.Popen("%s -ef|%s %s|%s -v grep|%s -l| %s -d ';\n'" % (SYSCMDS['ps'], SYSCMDS['grep'], SYSCMDS['tmm'], SYSCMDS['grep'], SYSCMDS['wc'], SYSCMDS['tr']),
...
def force_mgmt_mtu(mtu):
    ...
    fnull = open(os.devnull, 'w')
    subprocess.call([SYSCMDS['ip'], 'link', 'set', MGMT_DEV_NAME, 'mtu', "%s" % mtu],
                    stdout=fnull, stderr=fnull)
...
def make_dhcp4_request(interface, timeout=120):
    ...
    subprocess.call([SYSCMDS['ip'], 'link', 'set',
                     interface, 'up'], stdout=fnull)
    subprocess.call([SYSCMDS['dhclient'],
                     '-lf',
                     tmp_lease_file,
                     '-cf',
                     tmp_conf_file,
...
def get_configdrive_dev():
    ...
    dev = subprocess.Popen(
        "%s /dev/%s* | %s config-2 | %s -d':' -f1" % (
            SYSCMDS['blkid'], BLKID_PREFIX, SYSCMDS['grep'], SYSCMDS['cut']),
        stdout=subprocess.PIPE, stderr=fnull, shell=True
    ).communicate()[0].replace('\n', '')
    return dev
...
def mount_configdrive(dev, mountpath='/tmp/configdrive'):
    ...
    mountout = subprocess.Popen(
        "%s %s %s" % (SYSCMDS['mount'], dev, mountpath),
        stdout=subprocess.PIPE, stderr=fnull, shell=True
    ).communicate()[0].replace('\n', '')
    return mountout
...
def set_hostname(hostname):
    if not HOSTNAME_SET:
        wait_for_mcpd()
        fnull = open(os.devnull, 'w')
        cmd = "%s modify sys global-settings hostname %s" % (
        ...
        cmd = "%s mv cm device $(%s list cm device | %s 'cm device' | %s -d ' ' -f3) %s" % (
            SYSCMDS['tmsh'], SYSCMDS['tmsh'], SYSCMDS['grep'], SYSCMDS['cut'], hostname)
...
def run_modules():
    if os.path.exists(USERDATA_FILE):
        USERDATA_CONFIG = read_conf(USERDATA_FILE)
...
def onboard():
    if not os.path.exists(FIRST_BOOT_FILE):
        if onboard_configdrive():
            ...
            LOG.error(
                'BIG-IQ cloud-init only accepts ConfigDrive as a data source and none was found')
            touch_file(FIRST_BOOT_FILE)
    else:
