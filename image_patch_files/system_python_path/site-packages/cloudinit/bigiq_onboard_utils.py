...
def inject_public_ssh_keys(keys):
    ...
    if not os.path.exists(SSH_KEY_FILE):
        if os.path.exists('/var/ssh/root/authorized_keys'):
            os.symlink('/var/ssh/root/authorized_keys', SSH_KEY_FILE)
        else:
            LOG.error("root SSH key authorized_keys file missing.. not injecting SSH keys")
...
def create_cloudinit_resources(dev, systemid, userdata, metadata, networkdata,
    ...
    with open('/opt/cloud/data/previous-datasource', 'w') as ds:
        ds.write(ds_string)
    with open('/opt/cloud/instance/datasource', 'w') as ds:
        ds.write(ds_string)
